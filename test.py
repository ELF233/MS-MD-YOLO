...
def test(data,
    ...
    if wandb_images:
        wandb_logger.log({"Bo unding Box Debugger/Images": wandb_images})

    # Save JSON
    save_json = True
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights, list) else weights).stem if weights is not None else ''  # weights
        anno_json = 'E:/TBH/ir-reasoner-yolov7/yolov7-reasoner/train.json'  # annotations json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/flir.yaml', help='*.data path')
